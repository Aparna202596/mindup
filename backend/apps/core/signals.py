...
def topic_post_save(sender, instance, created, **kwargs):
    ...
    create_approval_request(
        object_type="topic", object_id=instance.id, user=instance.created_by
    )
    create_audit_log(
        user=instance.created_by, action="CREATE", object_type="Topic",
...
def category_post_save(sender, instance, created, **kwargs):
    ...
    from apps.core.services.approval_service import create_approval_request
    create_approval_request(
        object_type="category", object_id=instance.id, user=instance.created_by
    )
...
def subcategory_post_save(sender, instance, created, **kwargs):
    ...
    from apps.core.services.approval_service import create_approval_request
    create_approval_request(
        object_type="subcategory", object_id=instance.id, user=instance.created_by
    )
...
def answer_post_save(sender, instance, created, **kwargs):
    ...
    if instance.question.created_by_id and instance.question.created_by_id != instance.created_by_id:
        create_notification(
...
def answer_point_post_save(sender, instance, created, **kwargs):
    if not created:
        return
    from apps.core.services.notification_service import create_notification
    if instance.answer.created_by_id and instance.answer.created_by_id != instance.created_by_id:
        create_notification(
